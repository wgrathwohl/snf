...
def visualize_transform(samples, sample_names, logdensities, logdensity_names,
    ...
    #     plt_samples(data_samples, ax, npts=npts)
    # if z_samples is not None:
    #     ax = plt.subplot(1, 4, 2, aspect="equal")
    #     plt_samples(model_samples, ax, npts=npts, title="$x ~ p_\theta(x)$")

    # if z_data_samples is not None:
    #     ax = plt.subplot(1, 4, 3, aspect="equal")
    #     #ax.set_xlim(-2, 2)
    #     #ax.set_ylim(-2, 2)
    #     plt_samples(z_samples, ax, npts=npts, title="$x ~ p_\theta(x)$")
    #
    # if z_data_samples is not None:
    #     ax = plt.subplot(1, 4, 4, aspect="equal")
    #     #ax.set_xlim(-2, 2)
    #     #ax.set_ylim(-2, 2)
    #     plt_samples(z_data_samples, ax, npts=npts, title="$x ~ p_\theta(x)$")
    ...
    #     ax = plt.subplot(1, 4, 4, aspect="equal")
    #     plt_flow_density(logdensity, ax, npts=npts // 10, memory=memory, device=device, title="$q(z)$")
    # #
...
def visualize_slices(samples, sample_names,
    npts=100, memory=100, device="cpu", low=LOW, high=HIGH):
    # TODO: (MICHAL) - finish the function to work along with visualize_samples 
    """
    Produces visualization for the model density and samples from the model
